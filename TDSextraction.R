...
tdsExt <- function(df, att, sigLine, ...){
  ...
  #integrate stepwise
  is <- lapply(1:(nrow(df.e)-1), FUN = function(i) integrate(dd, lower = df.e$time[i], upper = df.e$time[i+1]))
  df.e$area <- c(0,unlist(lapply(1:43, FUN = function(i) is[[i]]$value))) #add to the df
  ...
  ExtParm <- cbind(areaT, areaPe, ddT, ddPe, tmaxT, tmaxPe, tAmaxT, tAmaxPe)
